...
static void spi_net_send_arp_request(struct net_device *dev, __be32 target_ip)
{
    ...
    memcpy(arp_ptr, src_mac, ETH_ALEN);       // sender MAC
    arp_ptr += ETH_ALEN;

    src_ip = 0;
    if (dev->ip_ptr && dev->ip_ptr->ifa_list)
        src_ip = dev->ip_ptr->ifa_list->ifa_address;
    memcpy(arp_ptr, &src_ip, 4);              // sender IP
    arp_ptr += 4;

    memset(arp_ptr, 0x00, ETH_ALEN);          // target MAC
    arp_ptr += ETH_ALEN;

    memcpy(arp_ptr, &target_ip, 4);           // target IP

    // SPI 전송 큐에 넣기
    if (priv->w_cnt < (SPI_BUF_NUM - 1)) {
        memcpy(priv->tx_buf[priv->w_cnt], buf, len);
        priv->w_cnt++;
...
static int spi_net_open(struct net_device *dev) {
    ...
    // priv->spi_pending = false;
    priv->rx_thread = kthread_run(rx_thread_fn, priv, "rx_thread");
    ...
    /* Start Dummy Data */
    for (i=0; i<10; i++) {
        spi_net_send_arp_request(dev, in_aton("192.168.0.1"));
    }
